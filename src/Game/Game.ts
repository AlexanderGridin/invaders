...
export class Game {
	...
	private spawnEnemy() {
		...
		const position = new Position(cell.position.x, cell.position.y - enemy.size.height);
		enemy.pushInGame(position, this.enemiesCounter.getValue());

		const isOutOfTheGrid =
			enemy.position.x < this.enemiesGrid.position.x || enemy.position.x + enemy.size.width > this.enemiesGrid.size.width;
		if (isOutOfTheGrid) {
			enemy.pullFromGame();
		}
